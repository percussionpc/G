Grovcoin Core version *0.15.1* is now available from:

  <https://download.grovcoin.org/grovcoin-0.15.1/>
...
A number of changes to the way Grovcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mGROV per kB (0.00001 GROV). This is to prepare for dropping the minimum transaction fee to 0.1 mGROV per kB (0.0001 GROV) in the next release.
